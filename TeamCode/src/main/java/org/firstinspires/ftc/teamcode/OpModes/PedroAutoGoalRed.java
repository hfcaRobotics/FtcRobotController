...
@Autonomous(name = "Roaring Auto Goal Red", group="AutoBot")
//@Disabled
public class PedroAutoGoalRed extends OpMode
{
   ...
   // private final Pose BlueScore = new Pose(27.8, 127.8, Math.toRadians(138)); // Scoring Pose of our robot. It is facing the goal at a 135 degree angle.
    private final Pose RedLoad = new Pose(126.8, 100, Math.toRadians(180)); // Highest (First Set) of Artifacts from the Spike Mark.
  //  private final Pose BlueLoad = new Pose(107.2, 11.6, 0); // Middle (Second Set) of Artifacts from the Spike Mark.
    ...
    private Path RedMove;
  //  private PathChain RedMove;


    public void buildPaths() {
        /* This is our BlueMove path. We are using a BezierLine, which is a straight line. */
        RedMove = new Path(new BezierLine(RedScore, RedLoad));
        RedMove.setLinearHeadingInterpolation(RedScore.getHeading(), RedLoad.getHeading());
