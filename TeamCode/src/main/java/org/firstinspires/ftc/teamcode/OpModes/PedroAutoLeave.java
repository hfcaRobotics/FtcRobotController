...
@Autonomous(name = "Roaring Auto Leave (face right)", group="AutoBot")
//@Disabled
public class PedroAutoLeave extends OpMode
{
   ...
   // private final Pose RedScore = new Pose(126.8, 124.9, Math.toRadians(42)); // Start Pose of our robot.
    private final Pose LeaveStart = new Pose(72, 11.6, Math.toRadians(0)); // Scoring Pose of our robot. It is facing the goal at a 135 degree angle.
 //   private final Pose RedLoad = new Pose(36.8, 11.6, Math.toRadians(180)); // Highest (First Set) of Artifacts from the Spike Mark.
    private final Pose LeaveEnd = new Pose(72, 41.6, 0); // Middle (Second Set) of Artifacts from the Spike Mark.
    private final Pose center = new Pose(72, 72, Math.toRadians(0)); // Lowest (Third Set) of Artifacts from the Spike Mark.

    private Path LeaveMove;
  //  private PathChain RedMove;


    public void buildPaths() {
        /* This is our BlueMove path. We are using a BezierLine, which is a straight line. */
        LeaveMove = new Path(new BezierLine(LeaveStart, LeaveEnd));
        LeaveMove.setLinearHeadingInterpolation(LeaveStart.getHeading(), LeaveEnd.getHeading());
    ...
    public void init() {
        ...
        buildPaths();
        follower.setStartingPose(LeaveStart);
